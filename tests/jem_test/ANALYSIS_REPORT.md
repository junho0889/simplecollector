...
## 7. 미션 1: Docker 통합 테스트 결과

### 7.1 테스트 환경
- **구성**: 14 Docker 컨테이너 (RabbitMQ + TimescaleDB + MC 시뮬레이터 + 10 Collector + 1 Publisher)
- **네트워크**: `jem-test-net` (bridge)
- **시뮬레이터**: MC Protocol Binary 3E 서버, 포트 5001-5010, test_data.csv 데이터 순환
- **DB**: TimescaleDB (schema: `jem_test`)

### 7.2 컨테이너 상태
| 서비스 | 상태 | 비고 |
|--------|------|------|
| rabbitmq | Healthy | AMQP 5672, 관리 15672 |
| timescaledb | Healthy | PostgreSQL 5432 |
| mc-simulator | Running | 10 PLC 포트 서빙 |
| collector-plc-a ~ plc-k (9개) | Healthy | 즉시 연결 성공 |
| collector-plc-l (PLC 10) | Healthy | 포트 오류(50010→5010) 수정 후 정상 |
| publisher | Healthy | master_sync 완료, 2개 큐 소비 |

### 7.3 발견 이슈
| 이슈 | 심각도 | 상태 |
|------|--------|------|
| collector_plc10.yaml `port: 50010` (오타) | 설정 오류 | **수정 완료** (→ 5010) |
| 에러 로그 | - | **없음** (전체 컨테이너) |
| 메시지 유실 | - | **없음** (RabbitMQ queue.db=0, queue.monitor=0) |

### 7.4 데이터 검증 결과

**DB 테이블**: 8개 생성 (plc_master, plc_data_master, plc_data_latest, plc_data_integrated, alm_master, alm_latest, alm_history, quality_master)

**plc_master**: 10개 PLC 등록 (ISA-95 계층 포함: 진영전기/릴레이 생산/JH-2HO)
...
| 2 (PLC-B) | 2,529 | ~6.5 | O |
| 3 (PLC-C) | 2,529 | ~6.5 | O |
| 4 (PLC-D) | 3,087 | ~7.9 | O (11태그) |
| 5 (PLC-EFG) | 1,686 | ~4.3 | O (6태그) |
| 6 (PLC-H) | 2,529 | ~6.5 | O |
| 7 (PLC-I) | 2,529 | ~6.5 | O |
| 8 (PLC-J) | 2,810 | ~7.2 | O (10태그) |
| 9 (PLC-K) | 2,529 | ~6.5 | O |
| 10 (PLC-L) | 279 | - | O (뒤늦게 시작) |
| **합계** | **23,036** | **~59/sec** | |

**알람**: alm_latest 526행, alm_history 526 이벤트 (ON 1건, OFF 525건)

### 7.5 스케일링 정확도 — test_data.csv vs DB 값 비교

**PLC 1 (PS 접점 조립)** — 9 태그 전수 비교:
| tag_id | 타입 | CSV 기대값 | DB 실측값 | 결과 |
|--------|------|-----------|-----------|------|
| 232 | uint32 (v_bigint) | 7230 | 7230 | **일치** |
| 233 | uint32 (v_bigint) | 7230 | 7230 | **일치** |
| 234 | uint32 (v_bigint) | 0 | 0 | **일치** |
| 235 | float32 (v_float) | 100 | 100 | **일치** |
| 236 | float32 (v_float) | 0 | 0 | **일치** |
| 237 | uint32 (v_bigint) | 7230 | 7230 | **일치** |
| 238 | int16 (v_int) | 18 | 18 | **일치** |
| 239 | int16 (v_int) | 0 | 0 | **일치** |
| 240 | uint32 scale=0.1 (v_float) | 3.3 | 3.3 | **일치** |

**PLC 4 (중간검사)** — 11 태그 전수 비교 (string 포함):
| tag_id | 타입 | CSV 기대값 | DB 실측값 | 결과 |
|--------|------|-----------|-----------|------|
| 326 | float32 | 288.41 | 288.41 | **일치** |
| 327 | float32 | 188.41 | 188.41 | **일치** |
| 328 | uint32 | 6803 | 6803 | **일치** |
| 329 | int16 | 11 | 11 | **일치** |
| 330 | int16 | 0 | 0 | **일치** |
| 331 | uint32 scale=0.1 | 5.2 | 5.2 | **일치** |
| 332 | uint32 | 2416 | 2416 | **일치** |
| 333 | uint32 | 837 | 837 | **일치** |
| 334 | float32 | 65.3 | 65.3 | **일치** |
| 335 | string | JH-2HO | JH-2HO | **일치** |
| 336 | uint32 | 0 | 0 | **일치** |

**PLC 5 (AIR 청소)** — 6 태그 전수 비교:
| tag_id | 타입 | CSV 기대값 | DB 실측값 | 결과 |
|--------|------|-----------|-----------|------|
| 103 | uint32 (v_bigint) | 42 | 42 | **일치** |
| 104 | uint32 (v_bigint) | 42 | 42 | **일치** |
| 105 | uint32 (v_bigint) | 42 | 42 | **일치** |
| 106 | int16 (v_int) | 22 | 22 | **일치** |
| 107 | int16 (v_int) | 0 | 0 | **일치** |
| 108 | float32 (v_float) | 1625336.2 | 1625336.2 | **일치** |

### 7.6 결론

**수집 파이프라인 정상 동작 확인**:
- 10/10 PLC 연결 및 수집 성공
- 스케일링 100% 정확 (uint32, int16, float32, string 모든 타입)
- 역스케일링(시뮬레이터→PLC raw값) → 정스케일링(collector) → DB 저장 전 과정 무결
- RabbitMQ 메시지 유실 0건
- 에러 로그 0건
- DB 크기: 스키마 1,168KB (6.5분, 23K행)
