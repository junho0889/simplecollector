...
BEGIN
    ...
    FROM jem_jh02.tb_prod_shift_history;
    IF v_cnt > 0 THEN
        RAISE NOTICE '기존 데이터 정리 중...';
        TRUNCATE jem_jh02.tb_prod_shift_history;
        TRUNCATE jem_jh02.tb_prod_hourly;
        TRUNCATE jem_jh02.tb_prod_daily;
        TRUNCATE jem_jh02.tb_prod_mode_change;
    ...
    -- tb_info_shift 확인
    SELECT COUNT(*) INTO v_cnt FROM jem_jh02.tb_info_shift;
    IF v_cnt = 0 THEN
...
SELECT
    ...
    (SELECT MAX(s2.shift_order) FROM jem_jh02.tb_info_shift s2 WHERE s2.line_id = sc.line_id) AS max_shift_order
FROM dates dt
CROSS JOIN jem_jh02.tb_info_shift sc
WHERE
    -- 미래 교대 제외
    CASE
        WHEN sc.start_time < sc.end_time THEN
...
DECLARE
    v_cnt INTEGER;
BEGIN
    INSERT INTO jem_jh02.tb_prod_shift_history (
    ...
    combined AS (
        ...
        LEFT JOIN jem_jh02.tb_info_target pt
    ...
    SELECT
        ...
        CASE WHEN (target_time_min - total_downtime_min) >= 10 THEN
            GREATEST(LEAST(ROUND((shift_production / ((target_time_min - total_downtime_min) * 60 / COALESCE(
                (SELECT cycle_time_sec FROM jem_jh02.tb_info_cycle_time
...
DECLARE
    v_cnt INTEGER;
BEGIN
    INSERT INTO jem_jh02.tb_prod_hourly (
...
DECLARE
    v_cnt INTEGER;
BEGIN
    INSERT INTO jem_jh02.tb_prod_mode_change (
...
BEGIN
    SELECT COUNT(*) INTO v_sh FROM jem_jh02.tb_prod_shift_history;
    SELECT COUNT(*) INTO v_hr FROM jem_jh02.tb_prod_hourly;
    SELECT COUNT(*) INTO v_dy FROM jem_jh02.tb_prod_daily;
    SELECT COUNT(*) INTO v_mc FROM jem_jh02.tb_prod_mode_change;
